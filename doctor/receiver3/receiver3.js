...
let DoctorTopic='DoctorTopic/'+'patient2'
let DoctorEmergencyTopic='DoctorEmergencyTopic/'+'patient2'
...
device.on('connect', function() {
    console.log('Connected to AWS IoT as receiver!');
    device.subscribe(DoctorTopic);
    //device.subscribe(DoctorEmergencyTopic)
    //device.subscribe(connecttodoctorTopic)
...
device.on('message', function(topic, message) {
    ...
    if (topic==firstjoinTopic){
        ...
        //le.log(jMessage1,backfirstjoinTopic,msg1)
        return
    }
    //console.log(connecttodoctorTopic+topic)
    if (topic==connecttodoctorTopic){
        let DoctorTopic='DoctorTopic/'+jMessage1['name']
        let DoctorEmergencyTopic='DoctorEmergencyTopic/'+jMessage1['name']
        //console.log('connect'+DoctorEmergencyTopic+DoctorTopic)
    }
    if (topic==DoctorEmergencyTopic)
    {
        //console.log('sendemail'+jMessage1+message)
        publishToSensorTopic('scalable/sink/email/alert', message);
        return
    }
